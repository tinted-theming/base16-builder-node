...
class BaseXTemplate {
  ...
  static populate (store) {
    ...
    const templateRoot = path.join(ROOTPATH, `${store}/templates`)

    return new Promise((resolve, reject) => {
      if (!file.exists(templateRoot)) {
        console.error(`ERROR: '${store}/templates' folder does not exist here.`)
        resolve([])
        return
      }
