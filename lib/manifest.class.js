...
class Manifest extends Reporter {
  ...
  update (targetPath) {
    ...
    const manifestPath = path.join(ROOTPATH, this.file)
    const manifest = file.loadFile(
      fs.existsSync(manifestPath) ? manifestPath : path.join(__dirname, this.file),
      yaml.load
    )
  ...
  refreshResources (dir, gitURL) {
    ...
    const reportChannel = Manifest.createChannel(`${resourcesExist ? 'Updating' : 'Fetching'} resources for ${dir}...`)
